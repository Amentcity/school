...
*   @RequestMapping标志一个类：设置映射请求的请求路径和初始信息
*   @RequestMapping标志一个方法:设置映射请求请求路径的具体信息
...
*   作用:通过请求的请求方式匹配请求
*   method属性是RequestMethod类型的数组，即当前浏览器发送请求的请求方式匹配method属性中的任何一种请求方式，
*   则当前请求就会被注解所标识的方法进行处理
...
*   ?:任意的单个字符(不包括?)
*   *:任意个数的任意字符(不包括?和/)
*   **:表示任意层数的任意目录,注意使用方式只能**写在//中，前后不能有任何的其他字符
...
//            params={"username","age=20","gender!=女"},
            headers = {"referer"}
    )
    public String hello(){
        return "success";
